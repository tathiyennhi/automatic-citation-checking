...
def _skip_ws_and_tags_right(text, pos):
    """Skip whitespace and [CITATION_X] tags going right."""
    while pos < len(text):
        if text[pos] in ' \t\n':
            pos += 1
        else:
...
def _skip_ws_and_tags_left(text, pos):
    """Skip whitespace and [CITATION_X] tags going left. Returns pos of last non-ws/tag char."""
    while pos >= 0:
        if text[pos] in ' \t\n':
            pos -= 1
        elif text[pos] == ']':
...
def _is_between_sentences(text, tag_pos, tag_len):
    """Tag is between sentences if preceded by '.' and followed by uppercase,
    skipping other [CITATION_X] tags and whitespace on both sides."""
...
def _extract_sentence_before_tag(text, tag_pos, tag):
    """Extract 'Sentence_A. [TAG] [consecutive tags...]' -- includes the tag
    and any immediately following citation tags. Finds sentence start by looking
    left for previous sentence boundary."""
    # End: tag + any immediately following tags (no trailing whitespace)
    ...
    # Go left past ws+tags to find '.' ending sentence A
    i = _skip_ws_and_tags_left(text, tag_pos - 1)
    # i is at '.' (verified by _is_between_sentences)

    # Find start of sentence A: search left for previous ". [skip tags] Uppercase"
    search = i - 1
    while search >= 0:
        if text[search] == '.':
        ...
        search -= 1
    # No previous boundary -> sentence starts at text beginning
    return text[0:end_pos]


def fix_spans(text, citation_spans):
    """Post-process spans:
    - Between sentences ("Sent A. [TAG] Sent B"): always extract "Sent A. [TAG]"
    - Mid-sentence: keep if valid, recover if span doesn't match text"""
    fixed = []
    for span in citation_spans:
        ...
        # Between sentences: extract sentence before + tag (STRICT RULE)
        ...
        # Mid-sentence: keep if span matches text
        if text.find(st) != -1:
            fixed.append(span)
            continue

        # Mid-sentence but doesn't match: try recovery (strip hallucinated tag)
