    ...
    for ref in references:
        ...
        if style == 'ieee':
            ...
            if match:
                ...
                print("THAM CHIẾU: ", ref, '\n')
                # print(key, "----", year, '----')
            else:
                print("Invalid", ref, '\n')
                # Xử lý các trường hợp không khớp với biểu thức chính quy
...
def find_best_match(citation, references, style='default'):
    if style == 'ieee':
        ...
        for ref in references:
            ref_authors = [author.strip().lower() for author in re.split(r',|\sand\s', ' '.join(ref[1]))]  # Kết hợp các tác giả thành một chuỗi
            ref_year = ref[2]
