...
# So sánh word embedding chi tiết và kiểm tra số liệu
for i, sentence_embedding in enumerate(high_similarity_word_embeddings):
    ...
    sentence_numbers = extract_numbers(high_similarity_sentences[i])
    if citation_numbers != sentence_numbers:
        print(f"Cảnh báo: Sự khác biệt đáng kể về số liệu. Citation đề cập đến {citation_numbers}, tài liệu gốc đề cập đến {sentence_numbers}.")
